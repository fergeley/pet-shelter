 ...
 *
 * Extracted from check-drift.mjs so it can be unit-tested. This is a safety
 * tool whose only unacceptable output is a false negative — telling an operator
 * that a `db push` which drops production tables is "safe to apply" — so the
 * classification rules need regression coverage, not just a careful read.
 */
 ...
 *
 * Comments are stripped from INSIDE each chunk rather than by discarding
 * chunks that begin with "--". Prisma labels every statement with a leading
...
export function stripBanner(stdout) {
  return stdout
    ...
    .filter(
      (l) =>
        !/^\s*(◇|Loaded Prisma config|Prisma schema loaded|Environment variables)/.test(l)
    )
    .join("\n")
    .trim();
}

export function classifyDiff(stdout) {
