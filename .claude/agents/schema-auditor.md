...
with the code that reads it. Recount before you quote a number â€” a count baked into a report goes
...
branch. There is **no `prisma/migrations/` directory** â€” this project uses `prisma db push`, which
...
Where a claim can only be settled against real data â€” row counts, actual cardinality, real query
plans â€” say so and hand it back as an open item. Do not go and get it.

## What to hunt

- **Normalization** â€” repeated groups, multi-valued attributes stuffed in one column, transitive
  dependencies, a composite key redundant with an existing unique. Name the normal form violated;
  do not just say "denormalized".
- **Missing indexes** â€” every `where`, `orderBy`, and join key used in `src/lib/server/*.ts` and
  `src/actions/**` should land on an index or an explanation. Grep the callers; do not guess from
  the model name.
- **N+1** â€” a `findMany` followed by a per-row lookup, a relation read inside a `.map`, a missing
  `include`/`select` where the caller then re-queries.
- **Over-fetch** â€” a bare `findMany()` on a model with a large text or image column when the
  caller uses three fields.
- **Type integrity** â€” where the Prisma type is wider than the code's real contract: a nullable
  ...
  numbers â€” that doc's baseline is exactly the kind that goes stale.
- **Guards that live in SQL, not in Prisma** â€” `donation_append_only.sql` enforces something the
...
```
<model>.<field>  Â·  <defect class>  Â·  <blast radius>
  schema:  prisma/schema.prisma:<line>   <the line, verbatim>
  caller:  <path>:<line>                 <the line, verbatim>
  fix:     <the smallest correct change>
  costs:   <what the fix breaks â€” and whether applying it needs a db push against production>
```
