...
**A4 retried rather than carried.** "The Prisma branch of `listPendingSponsorships` has never
run" was recorded as a gap because there is no Postgres here. That was a missing rung, not an
unverifiable claim. `tests/unit/sponsorships/pendingQueuePersistence.test.ts` stubs a dummy,
unreachable `DATABASE_URL` to flip the ledger persistent, doubles `@/lib/server/prisma`, and runs
the real branch: the `where`, the `orderBy`, the `select` that omits `taxIdOrIc`, and the
`Date`-to-ISO mapping the memory branch never exercises. Re-adding `taxIdOrIc: true` to the
`select` fails exactly the test that forbids it. **A4 is now half measured:** the query the code
sends and what it does with the answer. That Postgres executes it as written is still unobserved —
and, it turned out, was never blocked: see the correction under "Gates at close".

It lives in its own file for a reason worth keeping. Added to `reconciliation.test.ts` first, it
broke five unrelated tests with `Cannot read properties of undefined (reading 'create')`:
`vi.mock` is file-scoped, and `recordAuditLog` calls `prisma.auditLog.create` unconditionally, so a
double that knew only `petSponsorship` turned a swallowed async rejection into a synchronous
TypeError on every reconciliation.

**Master had moved under the branch.** `origin/master` rewrote `dal.ts` on 2026-09-10 ("a deleted
member loses the session, not just the row"): a missing member row no longer falls through to the
cookie, it consults the demo seed and denies any id the seed does not hold. The suites passed
there anyway — but only because the fixture's `usr-coord-01` happens to be a seeded id. Probed
with an unseeded coordinator id against master's `dal.ts`: the old `null` member default failed
**10 of 21**. The member double now echoes an ACTIVE row for whoever is signed in, and passes
with the unseeded id too. Agreement by design rather than by coincidence.

**One of this close-out's own checks was invalid, and was redone.** `FETCH_HEAD` was used as the
name for master; a later all-branch `git fetch origin` repointed it at this branch's own HEAD. One
"master compatibility" run therefore overlaid the branch's `dal.ts` onto itself and passed, and a
`todo.md` diff compared HEAD with HEAD. Both were redone against `origin/master` by name, with the
overlay first shown to differ (68 lines). The compatibility result held; the `todo.md` one did
not. Recorded as a lesson:
`tasks/lessons/2026-09-11-an-experiment-whose-input-equals-its-control-can-only-pass.md`.

**Gates at close:** `typecheck`, `lint` (17-warning baseline, none in changed files),
`docs:check`, unit 1305 (80 files), components 67 (6), integration 52 (6), and `npm run build`
with throwaway secrets inline and no env file present — `/admin/sponsorships` builds as `ƒ`.
`test:db` was not run.

*Correction, 2026-09-16:* the first draft of this line said `test:db` **cannot** run, because
Docker will not start here — the same wording every persistence entry had carried for eleven days,
this document's Build Gate included. It was false. `embedded-postgres` gives a real server with no
Docker in about four minutes, found by another session on 2026-09-14:
`tasks/lessons/2026-09-14-a-missing-docker-is-not-a-missing-database-tier.md`. "Not run" is the
accurate claim. The Build Gate's own wording above is left as it was written.

**Superseded as an implementation, not as a record.** The same close-out found that another
session built the same queue (`6fd1435`) three minutes before this branch's first commit, on a
branch that reached `master` through PR #36 on 2026-09-11. That implementation is the one to keep.
Re-read against `master` at `5b2672a` on 2026-09-16, it has also since fixed three of the four
defects this branch's reviews found — cookie-only guards, the post-mint "no sponsorship found",
and the catch claiming nothing was issued — and closed every gap this branch left out of scope.
One survives: the pending query materialises every supporter's NRIC. Everything above stays true
as a record of how the defects were found. What to do next:
[`../open/agent-7-branch-duplicates-the-reconciliation-queue-master-already-has.md`](../open/agent-7-branch-duplicates-the-reconciliation-queue-master-already-has.md).
