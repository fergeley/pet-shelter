/**
 * How a Postgres connection string is secured, decided from its host.
 *
 * Its own module, with no imports, so anything that opens a pool can share the
 * decision without instantiating the app's client: `src/lib/server/prisma.ts`
 * builds a `Pool` and a `PrismaClient` the moment it is imported, which is
 * why `prisma/seed.ts` and `scripts/migrate-faqs.ts` kept their own copies of
 * the substring sniff and its `rejectUnauthorized: false` after the app
 * stopped using it.
 */
