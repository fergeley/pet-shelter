...
import { sortFaqRecords } from "@/lib/domain/faq";
import { FAQ_CATEGORIES, FaqFormInput } from "@/lib/validations/faq";
import { FaqRecord } from "@/types/faq";
import { cn } from "@/lib/utils";

type CategoryFilter = (typeof FAQ_CATEGORIES)[number] | "all";
...
function matches(faq: FaqRecord, query: string): boolean {
  const q = query.toLowerCase().trim();
  if (!q) return true;
  const haystack = [faq.question, faq.answer, faq.questionMs, faq.answerMs]
...
export function FaqDataTable({ initialFaqs }: { initialFaqs: FaqRecord[] }) {
  ...
  /** Create/edit failures, shown inside the dialog that covers the banner. */
  ...
  const groups = useMemo(
    () =>
      FAQ_CATEGORIES.map((value) => ({
        value,
        label: FAQ_CATEGORY_LABELS[value].labelEn,
        rows: sortFaqRecords(filtered.filter((f) => f.category === value)),
        // Boundary checks use the unfiltered set so a hidden neighbour is still
        // recognised as a valid swap target.
        fullOrder: sortFaqRecords(faqs.filter((f) => f.category === value)),
  ...
  function handleOpenEdit(faq: FaqRecord) {
    setEditingFaq(faq);
    setSaveError(null);
    setIsFormOpen(true);
  }
  ...
  async function handleSave(data: FaqFormInput) {
    ...
    const result = editingFaq
      ? await updateFaqAction(editingFaq.id, data)
          ...
          {groups.map((group) => (
            <section key={group.value}>
              ...
              <div className="border border-border rounded-xl overflow-hidden bg-background divide-y divide-border">
                {group.rows.map((faq) => {
                  const position = group.fullOrder.findIndex((f) => f.id === faq.id);
                  const isFirst = position === 0;
                  const isLast = position === group.fullOrder.length - 1;
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 space-y-1">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="text-3xs font-bold uppercase tracking-wider text-muted-foreground tabular-nums border border-border px-1.5 py-0.5 rounded">
                            #{faq.displayOrder}
                          </span>
                          {!faq.isPublished && (
                            <span className="text-3xs font-bold uppercase tracking-wider bg-muted text-muted-foreground px-2 py-0.5 rounded">
                              Draft
                            </span>
                          )}
                          {!faq.questionMs && (
                            <span
                              className="text-3xs font-bold uppercase tracking-wider bg-muted/60 text-muted-foreground px-2 py-0.5 rounded"
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <Button
                          ...
                          size="xs"
                          onClick={() =>
                            run(() => toggleFaqPublishedAction(faq.id, !faq.isPublished))
                          }
                          disabled={isPending}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => {
                ...
                if (target) run(() => deleteFaqAction(target.id));
              }}
            >
              Delete permanently
            </Button>
