    ...
    <div className="space-y-2">
      <div className="flex items-start justify-between gap-3">
        ...
        {hasImage && !disabled && (
          <button
            type="button"
            onClick={handleClear}
            className="inline-flex items-center gap-1 text-3xs font-semibold text-destructive hover:underline shrink-0"
          >
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="space-y-2 min-w-0">
          ...
          {hasImage && (
            <p className="text-3xs font-mono text-muted-foreground truncate" title={value}>
              {value}
            </p>
          )}
        </div>
      ...
      {error && (
        <div className="flex items-start gap-2 p-2.5 bg-destructive/10 border border-destructive/30 rounded-lg">
          ...
          <p className="text-3xs text-destructive font-medium">{error}</p>
        </div>
      )}
    </div>
  );
