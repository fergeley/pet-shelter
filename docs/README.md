...
- **[✅ Target: Admin Status Parity](tasks/TARGET_ADMIN_STATUS_PARITY.md)**: Records P7 — the admin pet table renders rehabilitating animals as "Available" and its status filter cannot reach them. Admin-side follow-on to P5. Landed.
...
- **[✅ Target: Admin Status Write-back](tasks/TARGET_ADMIN_STATUS_WRITEBACK.md)**: Records P9 — `updatePetStatus` returns `{ success: false }` instead of throwing, the caller discards it, and the admin table persists a status the database refused. Resolved with snapshot rollback and form constraints.
...
- **[🟡 Target: Restructure Follow-Ups](tasks/TARGET_RESTRUCTURE_FOLLOWUPS.md)**: The two loose ends from the `src/lib/` work, each with a ready-to-paste dispatch prompt — wiring two components to the category readers nothing consumes, and making `verifyAdminSession()` return a principal so the audit trail can name who acted.
- **[🟢 Target: Medical Milestone & Clinical History Admin Editor](tasks/TARGET_MEDICAL_MILESTONE_ADMIN.md)**: Visual milestone list builder and recovery updates editor in `PetFormDialog.tsx` so staff can manage veterinary timelines and recovery notes directly in the admin portal.
- **[🟢 Target: Live Notifications Dispatcher & WhatsApp Routing](tasks/TARGET_LIVE_NOTIFICATIONS_DISPATCHER.md)**: Automated Resend application status emails, Malaysian phone number formatting, and coordinator WhatsApp deep-linking.
- **[🟢 Target: Shelter Analytics & Statutory LHDN Tax Audit Export](tasks/TARGET_SHELTER_ANALYTICS_AND_LHDN_EXPORT.md)**: LHDN Section 44(6) annual CSV export with RFC-4180 formula injection protection and live shelter capacity metrics.
- **[🟢 Target: Production Cloud Object Storage](tasks/TARGET_PRODUCTION_CLOUD_STORAGE.md)**: S3 / Cloudflare R2 / Supabase Storage adapter with magic-byte MIME validation and client-side WebP compression.
